...
Perform PCA over the input data ("Expression.RMA.txt"). By default it uses all the samples but you can choose a subset by unchecking "Select all samples".
```{r, echo=FALSE}
library(reshape2)
library(dplyr)
library(ggplot2)
library(knitr)
data.rma <- read.delim("/home/hugo/R/transcriptomics/Expression.RMA.txt") 
...
##Ten genes most negatively affected by PC1
```{r, echo=FALSE, message=FALSE, warning=FALSE}

renderTable({
  df<-rotation()
  df<-arrange(df, PC1)[1:10,]
...
##Ten genes most positively affected by PC1
```{r, echo=FALSE, message=FALSE, warning=FALSE}

renderTable({
  df<-rotation()
  df<-arrange(df, desc(PC1))[1:10,]
  select(df, Name, GeneName, Description, PC1)                
  })
